...
CREATE TABLE risks (
    ...
    rist_type               VARCHAR NOT NULL,          -- e.g. 'Financial','Non - Financial (IT Risk)'
    description             TEXT,
    risk_category           VARCHAR(100) NOT NULL,       -- from master_configurations
    coso                    VARCHAR(100),                -- e.g., 'Entity Level', 'Process level', 'ITGC level'
    risk_reference          VARCHAR(200),                -- e.g., 'ISO 27001 A.8.1', 'COSO ERM'
    risk_type               risk_type NOT NULL,          -- e.g. 'Fraud','Operating','Financial'
    -- process                 VARCHAR(200),
    ...
    risk_assertion_type 	VARCHAR(200),  -- 'Balance Sheet', 'Income Statement'
    risk_assertion     		VARCHAR(200),   -- 'Existence', 'Completeness', 'Accuracy', 'Valuation', 'Rights & Obligations'

    risk_owner          UUID REFERENCES users(id),   
    control_reference   VARCHAR(200),   
    control_description TEXT,
    control_type        control_type,
    prepared_by         UUID REFERENCES users(id),
    reviewed_by         UUID REFERENCES users(id),
    frequency          frequency_type,
    control_effectiveness_type VARCHAR(100), -- 'Design Effective','Operating Effective'
    control_effectiveness    BOOLEAN, -- true/false for effective/ineffective
    ipe   VARCHAR(200), -- Information Produced by Entity (IPE) reference
    
    --Only for IT/Cyber Risks
    cia_triad         VARCHAR(100),         
    risk_treatment    VARCHAR(100),
    -- Workflow
